        ...
        if actress_elements:
            for element in actress_elements:
                ...
                if self._is_valid_actress_name(actress_name):
                    results.append(
                        {"actresses": [actress_name], "source_element": "actress-name"}
                    )
        ...
        for article in articles:
            try:
                ...
                title_element = (
                    ...
                    or article.find("h2")
                    or article.find("h3")
                    or article.find(class_="entry-title")
                )
        ...
        try:
            ...
            if not result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"搜尋 AV-WIKI 影片 {video_code} 失敗: {result.error.message}",
                        {"url": search_url},
            ...
            # 處理搜尋結果
            if "unique_actresses" in parsed_data and parsed_data["unique_actresses"]:
                actresses = parsed_data["unique_actresses"]
                ...
                return Result.ok(
                    {
                        ...
                        "search_results": parsed_data.get("search_results", []),
                    }
                )
        ...
        try:
            ...
            if not result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"獲取 AV-WIKI 女優資訊 {actress_name} 失敗: {result.error.message}",
                        {"url": search_url},
                        result.error.caused_by,
                    )
                )
