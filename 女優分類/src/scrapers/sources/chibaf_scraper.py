        ...
        try:
            ...
            if not result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"搜尋 CHIBA-F 影片 {video_code} 失敗: {result.error.message}",
                        {"url": search_url},
            ...
            if "search_results" in parsed_data and parsed_data["search_results"]:
                # 尋找最匹配的結果
                best_match = None
                for item in parsed_data["search_results"]:
        ...
        try:
            ...
            if not result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"獲取 CHIBA-F 女優資訊 {actress_name} 失敗: {result.error.message}",
                        {"url": search_url},
                        result.error.caused_by,
                    )
                )
