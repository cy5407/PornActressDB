...
class UnifiedWebScraper:
    ...
    async def get_actress_info(
        ...
        # _search_from_multiple_sources now returns a dict of data, not Result objects directly
        # The success/failure is handled within _search_from_multiple_sources
        # So we can directly use 'results' here.
        ...
        async def search_single_source(source: DataSource) -> tuple:
            async with semaphore:
                try:
                    ...
                    # 設置超時
                    result: Result[Dict[str, Any]] = await asyncio.wait_for(
                    ...
                    if result.success:
                        ...
                        logger.warning(
                            f"⚠️ {source.value} 搜尋失敗: {query} - {result.error.message}"
                        )
                        return source, None
    ...
    async def batch_search_videos(
        ...
        for i in range(0, len(video_codes), batch_size):
            ...
            for j, result in enumerate(batch_results):
                code = batch_codes[j]
                # search_video_info now returns a Dict[str, Any] directly, not a Result object
                # The error handling is done within search_video_info and _search_from_multiple_sources
                # So we can directly assign the result
                all_results[code] = result

                if progress_callback:
                    status = "✅ 成功" if result.get("actresses") else "❌ 失敗"
