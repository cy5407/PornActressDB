...
class AsyncWebScraper:
    ...
    async def _make_request(
        ...
        try:
            ...
            if self.config.enable_cache:
                cached_content = await self.cache_manager.get_async(url)
                if cached_content:
                    self.stats["cache_hits"] += 1
                    logger.debug(f"📋 從快取獲取: {url}")
                    self._update_stats(domain, True, time.time() - start_time)
                    return Result.ok(cached_content)
            ...
            return Result.fail(
                ServiceError(ErrorCode.TIMEOUT_ERROR, error_msg, {"url": url})
            )

        except aiohttp.ClientResponseError as e:
            error_msg = f"HTTP 錯誤: {e.status} - {e.message} - {url}"
            logger.warning(f"🌐 {error_msg}")
            self._update_stats(domain, False, time.time() - start_time)
            return Result.fail(
                ServiceError(
                    ...
                    error_msg,
                    {"url": url, "status": e.status},
                    e,
                )
            )

        except aiohttp.ClientError as e:
            error_msg = f"網路連線錯誤: {e} - {url}"
            logger.warning(f"🌐 {error_msg}")
            self._update_stats(domain, False, time.time() - start_time)
            return Result.fail(
                ServiceError(ErrorCode.NETWORK_ERROR, error_msg, {"url": url}, e)
            )

        except Exception as e:
            error_msg = f"未知錯誤: {e} - {url}"
            logger.error(f"❌ {error_msg}")
            self._update_stats(domain, False, time.time() - start_time)
            return Result.fail(
                ServiceError(ErrorCode.UNKNOWN_ERROR, error_msg, {"url": url}, e)
            )
    ...
    async def _make_request_with_retry(
        ...
        """帶重試機制的請求"""
        last_result: Optional[Result[str]] = None
        ...
        return last_result or Result.fail(
            ServiceError(ErrorCode.UNKNOWN_ERROR, "所有重試都失敗", {"url": url})
        )

    async def scrape_multiple(
        self, urls: List[str], progress_callback: Optional[Callable[[str], None]] = None
    ) -> List[Result[str]]:
        """併發爬取多個URL"""
            ...
            async with semaphore:
                ...
                if progress_callback:
                    status = (
                        "✅ 成功"
                        if result.success
                        else f"❌ 失敗: {result.error.message}"
                    )
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    error_result = Result.fail(
                        ServiceError(
                            ErrorCode.UNKNOWN_ERROR, str(result), {"url": urls[i]}
                        )
                    )
    ...
    def _update_stats(self, domain: str, success: bool, response_time: float):
        """更新統計資訊"""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "cache_hit_rate": (
                f"{(self.stats['cache_hits'] / total_requests * 100):.1f}%"
                if total_requests > 0
                else "0%"
            ),
