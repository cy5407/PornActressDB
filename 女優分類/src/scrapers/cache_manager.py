    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_last_accessed ON cache_index(last_accessed)"
                )
        ...
        if self.config.enable_disk_cache:
            try:
                with sqlite3.connect(str(self.db_path)) as conn:
                    ...
                    if result:
                        file_path, created_at, ttl_seconds, compressed, access_count = (
                            result
                        )
    ...
    def delete(self, key: str) -> bool:
        ...
        try:
            ...
            if self.config.enable_disk_cache:
                with sqlite3.connect(str(self.db_path)) as conn:
                    cursor = conn.execute(
                        "SELECT file_path FROM cache_index WHERE key = ?", (cache_key,)
                    )
                    result = cursor.fetchone()
        ...
        try:
            ...
            if self.config.enable_disk_cache:
                with sqlite3.connect(str(self.db_path)) as conn:
                    ...
                    if expired_entries:
                        logger.info(
                            f"🧹 磁碟快取清理: {len(expired_entries)} 個過期條目"
                        )
        ...
        try:
            ...
            return {
                **self.stats,
                ...
                "memory_hit_rate": (
                    f"{(self.stats['memory_hits'] / total_requests * 100):.1f}%"
                    if total_requests > 0
                    else "0%"
                ),
                "disk_hit_rate": (
                    f"{(self.stats['disk_hits'] / total_requests * 100):.1f}%"
                    if total_requests > 0
                    else "0%"
                ),
