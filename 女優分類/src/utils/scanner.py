...
class UnifiedFileScanner:
    ...
    def scan_directory(self, path: str, recursive: bool = True) -> Result[List[Path]]:
        video_files = []
        scan_path = Path(path)
        if not scan_path.is_dir():
            ...
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"掃描路徑非資料夾: {path}", {"path": path}
                )
            )
        try:
            ...
            return Result.ok(list(set(video_files)))
        except Exception as e:
            logger.error(f"掃描目錄失敗: {e}")
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"掃描目錄失敗: {e}", {"path": path}, e
                )
            )
