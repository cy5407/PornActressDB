...
class PreferenceManager:
    ...
    def load_preferences(self) -> Result[Dict]:
        """載入使用者偏好設定 - 包含片商分類設定"""
        try:
            ...
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"載入偏好設定失敗: {e}", caused_by=e
                )
            )
    ...
    def save_preferences(self) -> Result[None]:
        """儲存偏好設定"""
        try:
            ...
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"儲存偏好設定失敗: {e}", caused_by=e
                )
            )

    def get_preferred_actress(self, actresses: List[str]) -> Result[Optional[str]]:
        """根據偏好選擇分類女優"""
        if not actresses:
            return Result.ok(None)
        ...
        if actresses_key in self.preferences["collaboration_preferences"]:
            return Result.ok(
                self.preferences["collaboration_preferences"][actresses_key]
            )

        # 優先級1：最愛女優
        for actress in actresses:
            if actress in self.preferences["favorite_actresses"]:
                return Result.ok(actress)
    ...
    def save_collaboration_preference(
        self, actresses: List[str], chosen: str
    ) -> Result[None]:
        """儲存共演組合的偏好設定"""
        try:
            actresses_key = "+".join(sorted(actresses))
            self.preferences["collaboration_preferences"][actresses_key] = chosen
            save_result = self.save_preferences()
            ...
            logger.error(f"儲存共演組合偏好失敗: {e}")
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"儲存共演組合偏好失敗: {e}", caused_by=e
                )
            )
    ...
    def set_solo_folder_name(self, folder_name: str) -> Result[None]:
        """設定單體企劃女優資料夾名稱"""
        try:
            self.preferences["solo_folder_name"] = folder_name
            return self.save_preferences()
        except Exception as e:
            logger.error(f"設定單體企劃女優資料夾名稱失敗: {e}")
            return Result.fail(
                ServiceError(
    ...
    def set_confidence_threshold(self, threshold: float) -> Result[None]:
        """設定片商信心度門檻"""
        try:
            ...
            logger.error(f"設定片商信心度門檻失敗: {e}")
            return Result.fail(
                ServiceError(
                    ErrorCode.FILE_ERROR, f"設定片商信心度門檻失敗: {e}", caused_by=e
                )
            )
