...
class SQLiteDBManager:
    ...
    def _create_schema(self):
        with self._get_connection() as conn:
            ...
            # 建立女優資料表
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS actresses (id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE)"
            )

            # 建立影片與女優關聯表（增強版 - 包含檔案關聯類型）
            cursor.execute(
                """CREATE TABLE IF NOT EXISTS video_actress_link (
                video_id INTEGER, 
                actress_id INTEGER, 
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_video_studio ON videos(studio)"
            )
            cursor.execute(
            ...
            if "studio_code" in current_columns:
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_video_studio_code ON videos(studio_code)"
                )
            ...
            if "file_association_type" not in link_columns:
                logger.info(
                    "升級 video_actress_link 表，添加 file_association_type 欄位..."
                )
                cursor.execute(
                    'ALTER TABLE video_actress_link ADD COLUMN file_association_type TEXT DEFAULT "primary"'
                )
            ...
            if "created_date" not in link_columns:
                ...
                cursor.execute(
                    "ALTER TABLE video_actress_link ADD COLUMN created_date TIMESTAMP"
                )
                # 為現有記錄設定預設時間戳
                cursor.execute(
                    "UPDATE video_actress_link SET created_date = CURRENT_TIMESTAMP WHERE created_date IS NULL"
                )

            # 建立新的索引以提升查詢效能
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_link_association_type ON video_actress_link(file_association_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_link_created_date ON video_actress_link(created_date)"
            )

            conn.commit()

    def add_or_update_video(self, code: str, info: Dict) -> Result[None]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                if video_row:
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            str(info.get("file_path")),
                            studio,
                            studio_code,
                            release_date,
                            info.get("search_method"),
                            datetime.now(),
                            video_id,
                        ),
                    )
                ...
                for name in actress_names:
                    cursor.execute(
                        "INSERT OR IGNORE INTO actresses (name) VALUES (?)", (name,)
                    )
                ...
                cursor.execute(
                    "DELETE FROM video_actress_link WHERE video_id = ?", (video_id,)
                )
                if actress_ids:
                    # 決定檔案關聯類型
                    for i, actress_id in enumerate(actress_ids):
                        ...
                        cursor.execute(
                            """INSERT OR IGNORE INTO video_actress_link 
                ...
                if studio or studio_code:
                    logger.info(
                        f"已更新番號 {code} 的片商資訊: {studio} ({studio_code})"
                    )
                else:
                    logger.debug(f"番號 {code} 未找到片商資訊")
                return Result.ok(None)
        except sqlite3.Error as e:
            logger.error(f"資料庫操作失敗 (add_or_update_video): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
            ...
            logger.error(f"未知錯誤 (add_or_update_video): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
    ...
    def get_video_info(self, code: str) -> Result[Optional[Dict]]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute(
                    "SELECT a.name FROM actresses a JOIN video_actress_link va ON a.id = va.actress_id WHERE va.video_id = ?",
                    (video_id,),
                )
            ...
            logger.error(f"資料庫操作失敗 (get_video_info): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.DATABASE_ERROR, f"資料庫操作失敗: {e}", {"code": code}, e
                )
            )
        except Exception as e:
            logger.error(f"未知錯誤 (get_video_info): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.UNKNOWN_ERROR, f"未知錯誤: {e}", {"code": code}, e
                )
            )

    def get_all_videos(self) -> Result[List[Dict]]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM videos")
                return Result.ok([dict(row) for row in cursor.fetchall()])
        except sqlite3.Error as e:
            logger.error(f"資料庫操作失敗 (get_all_videos): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.DATABASE_ERROR, f"資料庫操作失敗: {e}", caused_by=e
                )
            )
        except Exception as e:
            logger.error(f"未知錯誤 (get_all_videos): {e}", exc_info=True)
            return Result.fail(
                ServiceError(ErrorCode.UNKNOWN_ERROR, f"未知錯誤: {e}", caused_by=e)
            )

    def get_actress_statistics(self) -> Result[List[Dict]]:
        """取得女優統計資訊，包含片商分佈"""
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
            ...
            logger.error(f"資料庫操作失敗 (get_actress_statistics): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.DATABASE_ERROR, f"資料庫操作失敗: {e}", caused_by=e
                )
            ...
            logger.error(f"未知錯誤 (get_actress_statistics): {e}", exc_info=True)
            return Result.fail(
                ServiceError(ErrorCode.UNKNOWN_ERROR, f"未知錯誤: {e}", caused_by=e)
            )

    def get_studio_statistics(self) -> Result[List[Dict]]:
        """取得片商統計資訊"""
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
            ...
            logger.error(f"資料庫操作失敗 (get_studio_statistics): {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.DATABASE_ERROR, f"資料庫操作失敗: {e}", caused_by=e
                )
            ...
            logger.error(f"未知錯誤 (get_studio_statistics): {e}", exc_info=True)
            return Result.fail(
                ServiceError(ErrorCode.UNKNOWN_ERROR, f"未知錯誤: {e}", caused_by=e)
            )

    def get_enhanced_actress_studio_statistics(
        self, actress_name: str = None
    ) -> Result[List[Dict]]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                if actress_name:
                    ...
                    cursor.execute(
                        base_query
                        + " GROUP BY a.name, v.studio, v.studio_code, va.file_association_type ORDER BY video_count DESC",
                        (actress_name,),
                    )
                else:
                    cursor.execute(
                        base_query
                        + " GROUP BY a.name, v.studio, v.studio_code, va.file_association_type ORDER BY a.name, video_count DESC"
                    )
            ...
            logger.error(
                f"資料庫操作失敗 (get_enhanced_actress_studio_statistics): {e}",
                exc_info=True,
            )
            return Result.fail(
                ServiceError(
            ...
            logger.error(
                f"未知錯誤 (get_enhanced_actress_studio_statistics): {e}", exc_info=True
            )
            return Result.fail(
                ServiceError(ErrorCode.UNKNOWN_ERROR, f"未知錯誤: {e}", caused_by=e)
            )
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    studio_stats[studio]["codes"].extend(
                        codes.split(",") if codes else []
                    )
                ...
                for studio, stats in studio_stats.items():
                    ...
                    weighted_score = (
                        stats["primary_count"] * primary_weight
                ...
                if has_major_studio_work:
                    ...
                    if best_major_studio and best_major_studio == best_studio:
                        ...
                        if best_stats["total_count"] >= 3 and confidence >= 70:
                            ...
                            recommendation = "studio_classification"
                            confidence = max(confidence, 60.0)  # 提升20%信心度
                        else:
                            # 小片商作品過多（≥10部），分類為單體企劃
                            recommendation = "solo_artist"
            ...
            logger.error(
                f"資料庫操作失敗 (analyze_actress_primary_studio): {e}",
                exc_info=True,
            )
            return Result.fail(
                ServiceError(
            ...
            logger.error(
                f"未知錯誤 (analyze_actress_primary_studio): {e}", exc_info=True
            )
            return Result.fail(
                ServiceError(
                    ErrorCode.UNKNOWN_ERROR,
