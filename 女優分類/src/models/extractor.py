...
class UnifiedCodeExtractor:
    ...
    def __init__(self):
        ...
        self.code_patterns = [
            ...
            (r"([A-Z]{2,6}-\d{3,5})[A-Z]*", "標準格式帶後綴"),
            (r"([A-Z]{2,6}\d{3,5})", "無橫槓格式"),
            # 處理 STARS_707, STARS.707
            (r"([A-Z]{2,6}[._]\d{3,5})", "特殊分隔符格式"),
            (r"(\d{6}[-_]\d{3})", "數字格式"),
        ]

    def extract_code(self, filename: str) -> Result[Optional[str]]:
        """從檔案名稱提取番號"""
        try:
            # 取得不含副檔名的檔案名稱
            ...
            # 移除常見的品質和編碼標記
            # 移除 -C, CH, -C1 等結尾
            cleaned_name = re.sub(r"[-_]?[CHch]\d*", "", cleaned_name)
            # 移除 .H265
            cleaned_name = re.sub(r"\.H265", "", cleaned_name)
            cleaned_name = re.sub(
            ...
            logger.error(f"提取番號失敗: {e}", exc_info=True)
            return Result.fail(
                ServiceError(
    ...
    def _should_skip_file(self, base_name: str) -> bool:
        ...
        skip_markers = ["FC2PPV", "FC2-PPV", "FC2_PPV"]
        if any(marker in upper_name for marker in skip_markers):
            return True
