...
class StudioClassificationCore:
    ...
    def classify_actresses_by_studio(
        ...
        try:
            ...
            # 第一步：掃描所有女優資料夾
            actress_folders_result = self._scan_actress_folders(
                root_folder, progress_callback
            )
            if not actress_folders_result.success:
                return Result.fail(actress_folders_result.error)
            actress_folders = actress_folders_result.data
            ...
            # 第二步：重新掃描並更新統計資料
            updated_stats_result = self._update_actress_statistics(
                actress_folders, progress_callback
            )
            if not updated_stats_result.success:
                return Result.fail(updated_stats_result.error)
            updated_stats = updated_stats_result.data

            # 第三步：按片商分類移動
            move_stats_result = self._move_actresses_by_studio(
                root_folder, updated_stats, progress_callback
            )
            if not move_stats_result.success:
                return Result.fail(move_stats_result.error)
            move_stats = move_stats_result.data
            ...
            return Result.fail(
                ServiceError(
                    ErrorCode.UNKNOWN_ERROR, f"片商分類過程中發生錯誤: {e}", caused_by=e
                )
            )
    ...
    def _scan_actress_folders(
        ...
        try:
            ...
            return Result.ok(actress_folders)
    ...
    def _is_actress_folder(self, folder_path: Path) -> bool:
        ...
        try:
            ...
            if video_count >= 1 and (
                total_files <= 10 or video_count / total_files >= 0.3
            ):
                return True
    ...
    def _update_actress_statistics(
        ...
        for i, actress_folder in enumerate(actress_folders, 1):
            ...
            try:
                # 使用資料庫的增強分析功能
                analysis_result_res = self.db_manager.analyze_actress_primary_studio(
                    actress_name, self._major_studios
                )
                if not analysis_result_res.success:
                    self.logger.error(
                        f"資料庫分析女優 {actress_name} 失敗: {analysis_result_res.error.message}"
                    )
                    continue
                analysis_result = analysis_result_res.data
                ...
                if analysis_result["total_videos"] > 0:
                    ...
                    if video_files:  # 使用檔案掃描方式作為備援
                        studio_stats_res = self._calculate_studio_distribution(
                            video_files
                        )
                        if not studio_stats_res.success:
                            self.logger.error(
                                f"計算片商分佈失敗: {studio_stats_res.error.message}"
                            )
                            continue
                        studio_stats = studio_stats_res.data
        ...
        return Result.ok(updated_stats)

    def _calculate_studio_distribution(
        ...
        try:
            for video_file in video_files:
                # 提取番號
                code_result = self.code_extractor.extract_code(video_file.name)
                if not code_result.success or code_result.data is None:
                    logger.warning(
                        f"無法從檔案 {video_file.name} 提取番號: {code_result.error.message if code_result.error else '未知錯誤'}"
                    )
                    continue
                code = code_result.data

                # 識別片商
                studio_result = self.studio_identifier.identify_studio(code)
                if not studio_result.success or studio_result.data == "UNKNOWN":
                    logger.warning(
                        f"無法識別番號 {code} 的片商: {studio_result.error.message if studio_result.error else '未知錯誤'}"
                    )
                    continue
                studio = studio_result.data

                studio_stats[studio] += 1
            return Result.ok(dict(studio_stats))
        except Exception as e:
            logger.error(f"計算片商分佈失敗: {e}", exc_info=True)
            return Result.fail(
                ServiceError(
                    ErrorCode.PARSING_ERROR, f"計算片商分佈失敗: {e}", caused_by=e
                )
            )
