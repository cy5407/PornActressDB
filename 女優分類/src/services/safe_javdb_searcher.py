...
class SafeJAVDBSearcher:
    ...
    def safe_request(self, url: str, retry_count: int = 0) -> Result[httpx.Response]:
        """安全的 HTTP 請求方法"""
        with self._lock:
            # 檢查每日限制
            self._check_daily_reset()
            if self.stats["today_count"] >= self.daily_limit:
                ...
                return Result.fail(
                    ServiceError(
                        ...
                        f"已達每日 JAVDB 請求限制 ({self.daily_limit})",
                    )
                )
            ...
            try:
                ...
                if response.status_code == 429:  # Too Many Requests
                    ...
                    return Result.fail(
                        ServiceError(
                            ErrorCode.NETWORK_ERROR,
                            f"JAVDB 請求失敗: {response.status_code}",
                ...
                return Result.fail(
                    ServiceError(
                        ErrorCode.TIMEOUT_ERROR, "JAVDB 請求超時", {"url": url}
                    )
                )
                ...
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR, "JAVDB 連線失敗", {"url": url}
                    )
                )
    ...
    def search_javdb(self, video_id: str) -> Result[Dict[str, Any]]:
        """在 JAVDB 搜尋影片資訊"""
        if not video_id:
            return Result.fail(
                ServiceError(ErrorCode.VALIDATION_ERROR, "影片ID不能為空")
            )
            # 檢查快取
        cache_key = f"javdb_{video_id.upper()}"
        if cache_key in self.cache:
            logger.debug(f"📋 從快取取得 {video_id} 的 JAVDB 資料")
            return Result.ok(self.cache[cache_key])

        try:
            # 構建搜尋 URL
            search_url = f"https://javdb.com/search?q={quote(video_id)}&f=all"
            # 執行搜尋
            response_result = self.safe_request(search_url)
            if not response_result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"JAVDB 搜尋請求失敗: {response_result.error.message}",
                        {"url": search_url},
                        response_result.error.caused_by,
                    )
                )
            ...
            # 訪問詳情頁面
            detail_response_result = self.safe_request(detail_url)
            if not detail_response_result.success:
                return Result.fail(
                    ServiceError(
                        ErrorCode.NETWORK_ERROR,
                        f"JAVDB 詳情頁面請求失敗: {detail_response_result.error.message}",
                        {"url": detail_url},
                        detail_response_result.error.caused_by,
                    )
                )

            detail_response = detail_response_result.data
    ...
    def _parse_detail_page(
        ...
        try:
            ...
            if soup is None:
                ...
                return Result.fail(
                    ServiceError(
                        ErrorCode.PARSING_ERROR, f"無法解析 JAVDB 詳情頁面: {url}"
                    )
                )
